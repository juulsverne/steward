...
| NFR-06 Bounded work and cost visibility | End invocations at their stopping conditions; never loop on unchanged denials. A denial can lead to escalation in the same run. Each invocation has lifetime limits of 12 model cycles and 40 requested tools, with up to 3 attempts and an immutable deadline/key per logical external request. The initial execution episode is 120 seconds. One additional 120-second recovery episode is permitted only for genuinely interrupted unfinished processing, retaining the same invocation/trigger, lifetime counters and original request deadlines. Unexpired takeover gets only the remaining time; normal waits, completion, acknowledged malformed output and explicit exhaustion get no fresh episode. This approved B12 adjustment covers payment committing before a crash and before the model has chosen RESOLVE; receipt reconciliation does not choose a business action. No sliding or third episode. Record latency/usage, do not promise a dollar cost or unlimited retries | B10–B12; P8; H2/H4 |
