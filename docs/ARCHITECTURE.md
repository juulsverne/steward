...
- **Deployed EC2 / CloudFront:** CloudFront supplies the public HTTPS origin and forwards to nginx on one EC2 instance. FastAPI serves React static files and all policy-bearing API operations. SQLite, images and pending work use the retained data volume; a scheduled job copies consistent backups to private S3. The agent runs in-process and uses the same HTTP tools as local execution. See [HOSTING_DECISION.md](HOSTING_DECISION.md) for exact resources and the approved $50 ceiling.
- **Observed deployment evidence:** application commit `57e7e27` passed hosted criteria 1–15, service restart and instance reboot with the resolved case and one simulated payment preserved. Hosted criterion 16, instance replacement, backup restoration and unfinished-work recovery remain unverified. Later local UI/script fixes require separate deployment verification.
- **Unbuilt AgentCore Runtime / Observability / Gateway:** these remain tier-3 tasks. Runtime would host the same investigator and call the existing API tools; Observability would correlate cloud traces; Gateway would expose existing operations as MCP. None is part of the deployed topology or a claimed passed gate.

App Runner was superseded by the approved EC2 arrangement. The [build guide](BUILD_PLAN.md) retains the original tier plan and historical checkpoints; the [hosted receipt](evaluations/2026-09-14-hosted-acceptance.md) is the authority for what actually ran.
