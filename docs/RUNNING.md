...
[Back to the project overview](../README.md). These instructions cover the existing prototype. Run commands from the repository root. Live acceptance receipts are dated September 14, 2026; they are historical results, not a new validation of your environment.

## Run it yourself

You need Python 3.11+, [uv](https://docs.astral.sh/uv/), Node.js **22.22.2+ in the 22.x line or 24.15+ in the 24.x line** with npm, the AWS CLI, and your own AWS account with Amazon Bedrock access to `global.anthropic.claude-sonnet-4-6`. The client uses `us-west-2`. Live runs incur inference costs; the screenshot walkthrough in the README does not require AWS credentials. Commands below use Bash (Git Bash on Windows, or a macOS/Linux terminal). Start the API in one terminal and run the driver in a second terminal from the repository root.
...
The same journey can be walked by hand: the Report form submits the second resident report, the Crew Form accepts, checks in (a button fills the dispatch coordinates) and uploads `data/images/middle.jpg` then `data/images/after.jpg`, and the Inbox offers **Request completion** on the failed-proof exception. The recorded end-to-end evidence is the driver; the manual path is covered by the frontend tests and the walk's read-only screens, not by a recorded hand-driven run.
...
Reset refuses unmarked files and never touches `.env`. For an isolated, reproducible acceptance run on a private port (seed, serve on 8001, drive, stop, keep logs under `.steward/b13/`), use `bash scripts/acceptance_run.sh <tag> 8001`; pass a previous artifact as the third argument to close criterion 16 by comparison. Details in [DEMO.md](DEMO.md#live-acceptance-procedure-b13-api-gate).
...
Interrupted runs are safe to resume: pending work, claims and effects are journaled, and a restarted API resumes or fails them without duplicating a job, reservation or payment ([API.md](API.md#durable-processing-and-recovery-b12)).
