...
The vision CLI deliberately reports `FAIL`/`passed: false` with `--repeats 1`, including Sonnet: its qualification gate requires at least three repeats. The 4/4 screen is not that gate. The earlier Sonnet **12/12** component qualification remains separately documented in [VISION_SPIKE.md](VISION_SPIKE.md); it is neither a new result nor proof of the whole workflow.
