...
Status, September 14, 2026: **API gate (B13) passed** in independently seeded runs 16 and 20. **UI gate (P7) remains open**: the [UI walk](evaluations/2026-09-14-ui-walk.md) records rendered states from an API-driven journey, with operator-action coverage in tests; it is not a complete browser-driven repeat. EC2/CloudFront hosting is deployed at application commit `57e7e27`; [hosted criteria 1–15, service restart and reboot passed](evaluations/2026-09-14-hosted-acceptance.md). Hosted criterion 16, instance replacement and backup restoration remain unverified. The 22-scenario evaluation has not run. Later local fixes require a separate deployment and verification.
...
2. `cd frontend && npm ci && npm run build && cd ..` so the API serves the screens at `http://localhost:8000/`.
...
| 0:25–0:50 | 2. `architecture.png` | Point at the three boxes | One Strands agent on Bedrock chooses what to do next through typed HTTP tools. FastAPI owns policy, scores, prices, budget, verification and settlement; the agent never opens the database. A trusted Bedrock vision call inspects each photo and returns findings that code scores. The same system is hosted for judging on EC2 behind CloudFront (deployed today; its acceptance evidence is a separate file); AgentCore is not deployed. This recording runs locally. | design | "Hosted: EC2 + CloudFront · AgentCore: not deployed" |
...
| 1:20–1:35 | 4. Terminal (Take A) | Start the driver; show the `criterion-1` and `criterion-2` lines, then leave it running | This is a live run: real Bedrock calls, synthetic photos, seeded fixtures, simulated dispatch and settlement. The driver submits the resident, crew and operator actions through the same API the screens use; nothing edits the database by hand. | run is live | "Live run — recorded September 14, cut for time" |
| 1:35–2:05 | 5. Issue Detail `/issues/demo-couch` | Scroll from the top: sources with provenance tags, evidence points table, the conflict block | The driver submitted a second resident observation through the API — that source is labeled live, the first is seeded. Two observations newer than the city's completion time confirm the dispute; the record is now credited and the score is 100. Completion time and observation times sit side by side; the issue remained open until the later accepted proof. | 3, 4, 5 | "seeded" / "live" provenance tags on screen |
...
| 3:00–3:25 | 8. Issue Detail, expanded decision history and proof history | Show the recorded operator request and rework event, then the fresh after proof at 100 of 95. The final paid state no longer offers Request completion or a rework form. | The operator's only action at that point was Request completion. It resumed the agent with a rework instruction on the same job, same $72, no second reservation. Fresh proof passed the scene and reuse prerequisites and scored 100. | 11, 12 | "Same job, same quote" |
...
- [Clean-install result](evaluations/2026-09-14-clean-install.md): 706 backend tests and frontend checks passed on `57e7e27`. [Judging URL](https://d1uke66gfefpu4.cloudfront.net) is live; public video remains pending. See [SUBMISSION.md](SUBMISSION.md).
