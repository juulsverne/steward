...
| Bedrock (Sonnet 4.6 text + vision, per use) | **Measured** with CloudWatch `AWS/Bedrock` for `global.anthropic.claude-sonnet-4-6`: the seeded intake invocation plus the full acceptance walk (16:00–16:10 CDT buckets) used 60 model invocations, about 1.16 M input tokens and 17 K output tokens; at $3 / $15 per million that is **≈ $3.75 per full couch walk** (image tokens dominate; the M0 planning example of $0.0225 per call undercounts vision calls). The driver's own five-minute bucket alone was 36 invocations / 772 K input / 10 K output ≈ $2.47 | ≈ $3.75 per complete judge walk; reads (Board, Issue Detail, Inbox) invoke nothing. The $15 allowance covers about four full walks; the $5 contingency about one more |

Standard CPU credits cannot generate surplus charges; the instance idles at a few percent CPU and 270 MB RAM. Stopping the instance saves only the compute line; EBS and the public IPv4 address keep billing until released. Infrastructure (~$17) plus four full walks (~$15) stays inside the approved $50 ceiling (the H1 split was $30 hosting/retention, $15 model use, $5 contingency); every additional full walk costs ≈ $3.75, so more than about eight judge walks would need the owner to raise the ceiling. AWS Budgets alerts at $25/$35/$45 are recommended but were not created (a Budget is outside the approved resource list; the owner can add one in the console). The same account also runs local acceptance walks by other workers; the CloudWatch metric does not separate them from the hosted instance, so reconcile against the billing console.
