...
> For agentic workers: when the user starts the build, use `superpowers:subagent-driven-development` to delegate bounded implementation tasks and independent reviews. Follow the model assignments and ownership rules in section 12. This revision prepares the plan for owner review; it does not start implementation. Steward itself runs exactly one Strands agent.
...
**Tech stack:** the existing Python package, Strands, Bedrock models selected for task quality and cost, FastAPI, SQLite, Pillow and Pydantic; then React/Vite and Leaflet; then AgentCore Runtime, Observability, App Runner and Gateway, subject to the storage decision below. Sonnet is the current code default; per-role settings and promotion are planned in M0.
...
- [ ] Add explicit text/image model settings with the existing single-model setting as a fallback; log the actual resolved IDs. Keep Sonnet as the default while replacements are evaluated. Test that a text setting change cannot silently replace the inspector.
...
- [ ] Keep SQLite details inside Store/migrations; tools and screens depend on typed records and API operations. Before dispatching dependent workers, publish the actual request/response models, foreign-key/uniqueness rules and method signatures in `contracts.py` and OpenAPI as applicable. Freeze one owner for changes to shared contracts.
