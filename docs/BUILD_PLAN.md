...
- Operational limits: twelve model cycles and forty requested tools for the entire saved invocation; three attempts and an immutable key/deadline for each logical external request. The initial episode is120seconds. Approved B12 recovery adjustment: one additional120-second episode only for genuinely interrupted unfinished processing, retaining the invocation/original trigger and lifetime counters. Takeover within an unexpired episode receives its remainder. Normal waits/completion, acknowledged malformed output and explicit exhaustion cannot restart; no sliding or third window. The offline real-API crash test covers payment committed before acknowledgment and before any model RESOLVE choice, followed by recovery after five minutes with one payment and a newly model-chosen RESOLVE. Read-only receipt reconciliation never replenishes the original request's deadline/attempt budget or invents that choice. This is engineering recovery evidence, not a measured live-latency claim. Record settings and adjust from observed failures, not guesses. Do not change scores to make a run pass.
...
**Implemented files:** `src/agent/runtime.py`, `dispatcher.py`, `coordinator.py`, `runtime_api.py`, `runtime_contracts.py`, and `tools/durable.py`; extend API, Store, migrations, hooks, CLI and configuration. Regression coverage is in `tests/test_runtime.py`, `test_runtime_recovery.py`, `test_runtime_ack_recovery.py`, and `test_runtime_atomic_fence.py`.

- [x] Persist each accepted trigger's invocation and process it through `run_invocation`, with pending/running/waiting/completed/error records and bounded claims/leases. Startup discovery recovers accepted unfinished events without a new broker.
- [x] Acknowledge intake/proof/operator events after saving them. Process outside request write transactions and without blocking the API event loop. Accept/check-in make no model calls.
- [x] Enforce section 6 caps/stops; record actual tool requests/results, denied attempts, decisions, versions, usage, latency and safe failures under one invocation ID. Mutation endpoints enforce scope even if a steering hook is bypassed.
- [x] Verify durable startup, waiting/restart, competing claims, denied credentials, malformed output, acknowledgment loss, observed timeout and paid-before-close interruption. Run `uv run --no-sync python -m pytest -p offline_guard tests/test_runtime.py tests/test_runtime_recovery.py tests/test_runtime_ack_recovery.py tests/test_runtime_atomic_fence.py -q`.
...
- [x] Built `python -m agent.demo --base-url URL --out PATH [--compare PRIOR]` plus `scripts/acceptance_run.sh <tag> [port] [compare]`, which seeds an isolated store, starts the API with the runtime on a private port, drives the actor events and stops that server. The driver grades saved results only; it never selects tools or inserts decisions.
- [x] Ran the real preflight and the live sequence 65 wait → 85 → 100 dispute → $72 dispatch → 90 denied settlement → operator decision → rework → 100 → one payment → resolution with Sonnet text and vision: runs 13, 16, 17, 19 and 20 on September 14 each passed criteria 1–15 with five invocations and no invocation error (`.steward/b13/run16-acceptance.json` is the retained first run).
- [x] Criteria 5/15 are verified from the Issue Detail and Board projections in the artifacts; their screen rendering remains open for P7. DEMO negatives are covered by the guarded suite (direct-call denials, concurrent spending, stale proof/decision, blocked uploads while awaiting an operator, interrupted closure) plus the live mixed-hazard intake check and the twelve-comparison vision spike.
- [x] Reset an independent dataset and reran with `--compare`: run 20 closed criterion 16 against run 16 with equivalent normalized judgments (`.steward/b13/run20-acceptance.json`). Per-step API responses, every invocation's model/tool observations, usage, versions and all failed attempts (runs 5–15, 17–19) are retained under `.steward/b13/`. Ruff passes; the full guarded suite result is recorded in the B13 receipt below.
...
**API gate passed** on September 14 (runs 16 and 20); UI is not passed. Tier 2 may start.
...
- [ ] Reset a named sandbox and perform all sixteen steps using the screens, real Strands/vision and persona events. Inspect network errors and browser console. The earlier checkbox was reopened on September 14: the retained walk used the API driver and read-only UI inspection, not all browser actions.
...
| H1 | Owner approved EC2/EBS/S3/CloudFront and $50 ceiling; deployment recorded September 14 | [Hosting decision](HOSTING_DECISION.md) and [hosted evidence](evaluations/2026-09-14-hosted-acceptance.md): criteria 1–15, resolved-state restart/reboot passed. Replacement, backup restoration and hosted repeat remain open; AgentCore not deployed |
...
### B12 completion receipt — September 13

B12 is complete locally after an Astra high implementation, independent Astra high runtime review, and lead integration review. FastAPI owns schema 7, bounded claims, durable command/attempt journals and safe status/trace projections. The HTTP-only runner discovers persisted work, preserves original request identity through fenced recovery, and keeps ordinary waits and observed terminal errors dormant. Mutation authority is checked before replay and again at transaction commit. A real API test verifies payment committed before interruption, recovery after five minutes, and a subsequent model-selected RESOLVE with one payment; its model is injected, so this is offline recovery evidence.

The full guarded suite recorded 660 passes and 12 legacy test-fixture failures in 765.86 seconds. No application code changed afterward. Corrected fixtures and their consumers passed 41 checks in 34.64 seconds; the final exact-key replay regression then passed independently as a single maintained check. Independent probes passed all three lost-acknowledgment cases plus the observed deadline case (four checks in 6.11 seconds), and the lease-expiry transaction probe passed in 1.15 seconds. All 41 final candidate hashes matched; scoped Ruff and whitespace checks passed. The original full-run failures remain part of the evidence, rather than being relabeled as a fresh all-pass run.

The real intake component was separately requalified after strict native-JSON parsing and a measured truncation repair: the unchanged prompt/schema with a 1024-token cap produced nine valid responses, all semantically reviewed. Earlier failures are retained. This does not close B13's real sixteen-step agent/API gate, frontend work or hosting. Enable local processing explicitly with `STEWARD_RUNTIME_ENABLED=true`; see [API.md](API.md) for startup, status and recovery contracts.

### B13 completion receipt — September 14

B13 is complete locally: the sixteen-step couch journey passed through the API with real Sonnet text and vision in run 16 (criteria 1–15, three minutes, five invocations, about 574k input tokens) and again in the independently reseeded run 20, which also closed criterion 16 by comparing normalized judgments (issue RESOLVED at 100, job PAID once for 7,200 cents, one accepted proof after one rejected 90-point proof, budget 42,800 available and 7,200 spent, Board resolved marker). Artifacts, server/driver logs and stores for every attempt are under `.steward/b13/`; `scripts/acceptance_run.sh` reproduces a run on an isolated store and port.

Getting there required nine live-observed repairs, each fixed at its root with an offline test before the next run: (1) similar-issue candidates and case-packet issue candidates now carry `state_revision`, so an unlinked signal's link request can cite the real revision instead of guessing (run 5's twelve-cycle conflict loop); (2) the `aws login` credential refresh failed whenever a session was pinned to the Bedrock region, because botocore refreshes through the sign-in endpoint of the session's region; credentials now resolve in the profile's configured region (`agent.aws_session.region_session`) and the preflight uses the same path; (3) `MARK_ACTIONABLE` denials name each blocking fact (`retained_hazards_require_operator_review`, `classification_unknowns_unresolved`, `jurisdiction_unknowns_unresolved`) and the classify/jurisdiction/decision tool contracts define hazards, unknowns and permitted decision types; (4) the intake vision contract defines `visible_hazards` as specialist safety hazards and `unknowns` as scope-blocking gaps, so the couch photo inspects to none while `mixed-hazard.jpg` still reports its cable; (5) a saved `REQUEST_OPERATOR` intent posted to the investigation action is denied with `request_operator_uses_escalate_to_operator` instead of a run-ending 422; (6) plan denials tell the investigator which facts it can repair and which require escalation; (7) a nested `basis` object the model serialized as a JSON string is parsed at the tool boundary, and the operational decision contract enumerates basis kinds per decision type; (8) a malformed tool call now cancels only that call with its validation reason and spends one request slot instead of ending the invocation; (9) the completion vision contract defines a new hazard as one introduced by the work, requalified by the twelve-comparison spike (12/12, `.steward/vision-spike-20260914T073629760499Z.json`) after one run scored the middle proof 80. The driver itself had two latent defects that only a completed run could reveal: it compared artifacts before stamping its own completion time, and it compared per-run submission IDs; both are fixed with tests.

Versions in the passing runs: prompt `steward-investigator-v2`, tools `steward-http-tools-v2`, intake prompt `d7593b85983eb362`, intake schema `a547a431f79dbddf`, completion prompt `cea02b88fbad2ce3`, completion schema `6d1d3b188972ac46`, policy `south-loop-v3`, seed `south-loop-demo-b3`. Known limits: Bedrock workloads must run one at a time (a concurrent vision spike caused a `ThrottlingException` in run 15 because SDK retries are disabled); the model occasionally spends a cycle on a recoverable slip (wrong rework decision id in run 13, wrong basis kind in run 18), now bounded by the cycle and request limits rather than fatal. Ruff passes on `src` and `tests`. The full guarded suite on the final code passed: `uv run --no-sync pytest -q` — **703 passed, 1 warning in 754.84s (0:12:34)**.
