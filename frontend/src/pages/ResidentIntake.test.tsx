...
let posts: FormData[] = []; let fail = false; let receiptError = false;
beforeEach(() => { resetSessionForTests(); posts = []; fail = false; receiptError = false; vi.spyOn(globalThis, "fetch").mockImplementation((input, init) => {
  ...
  if (url.includes("/receipt")) return receiptError ? Promise.reject(new Error("network down")) : Promise.resolve(envelope({ processing: "COMPLETED", invocation_id: "inv-2", updated_at: "2026-09-14T15:01:00Z", reason_code: null }));
...
describe("ResidentIntake", () => {
  ...
  it("keeps the receipt visible and retries only its status when polling fails", async () => {
    receiptError = true; mount();
    await userEvent.type(await screen.findByLabelText("What did you see?"), "Couch");
    await userEvent.type(screen.getByLabelText("Where?"), "1200 S Wabash Ave");
    await userEvent.click(screen.getByRole("button", { name: "Send report" }));
    expect(await screen.findByText("rcpt-1")).toBeInTheDocument();
    expect(screen.getByText("Report saved; status unavailable")).toBeInTheDocument();
    receiptError = false;
